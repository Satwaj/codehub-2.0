    ...
    <div className="flex gap-4">
      {Object.entries(timeLeft).map(([label, value]) => (
        <div key={label} className="text-center">
          ...
          <span className="text-[10px] text-text-secondary uppercase tracking-[0.2em] mt-3 block font-bold">
            {label}
          </span>
        </div>
      ))}
      ...
      <div ref={sectionRef} className="section-inner z-10">
        ...
        >
          ...
          <h2 className="heading-display text-5xl md:text-6xl lg:text-7xl xl:text-8xl mt-4">
            Upcoming{" "}
            <span className="italic font-serif text-white/60">Residencies</span>
          </h2>
        </motion.div>
        ...
        {UPCOMING_EVENTS.map((event, i) => (
          <motion.div
            ...
            className="glass-card relative overflow-hidden p-0 mb-10 max-w-[1000px] mx-auto"
          >
            ...
            <div className="p-10 md:p-16 relative">
              ...
              <div className="flex flex-col items-center justify-between gap-12 text-center relative z-10">
                <div className="flex-1 w-full">
                  ...
                  <h3 className="text-5xl md:text-6xl lg:text-7xl font-serif font-light mb-8 text-transparent bg-clip-text bg-gradient-to-b from-white to-white/60">
                    {event.season}
                  </h3>
                  ...
                  <div className="flex flex-wrap justify-center gap-3 mb-10">
                    {event.features.map((f, j) => (
                      <span
                        key={j}
                        className="px-5 py-2 rounded-full border border-white/10 bg-white/5 text-white/80 text-sm font-light backdrop-blur-sm"
                      >
                        {f}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-col items-center gap-6 mt-12 bg-bg-dark/50 py-8 rounded-3xl border border-white/5 backdrop-blur-md">
                    <span className="text-xs text-text-secondary uppercase font-bold tracking-[0.2em]">
                      Program Fee
                    </span>
                    <div className="text-5xl md:text-6xl font-serif font-light text-white">
                      {event.price}
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-2 mt-4 w-full px-8">
                      <div className="flex justify-between w-full max-w-sm text-sm font-medium">
                        ...
                        <span className="text-primary font-bold">
                          {event.seatsLeft} / {event.totalSeats}
                        </span>
                      </div>
                      <div className="w-full max-w-sm h-1.5 bg-white/10 rounded-full overflow-hidden mt-1">
                        <div
                          className="h-full bg-gradient-to-r from-red-500 to-primary rounded-full transition-all duration-1000"
                          style={{
                            width: `${(event.seatsLeft / event.totalSeats) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center gap-12 mt-4 pt-12 w-full max-w-sm relative">
                  ...
                  <a
                    href="#cta"
                    className="btn-gold group relative overflow-hidden w-full justify-center"
                  >
