  ...
  return (
    <SectionWrapper className="section-dark-2 relative overflow-hidden">
      ...
      <div ref={sectionRef} className="section-inner z-10">
        ...
        >
          <span className="glass-pill mb-6 text-accent-blue border-accent-blue/30">
            WHO CAN APPLY?
          </span>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {PERSONAS.map((persona, i) => (
            ...
            >
              <div
                className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r ${persona.color} opacity-40 group-hover:opacity-100 transition-opacity`}
              />
              <div className="mb-10">
                <div
                  className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${persona.color} p-[1px] mb-8`}
                >
              ...
              <ul className="space-y-4">
                {persona.points.map((pt, j) => (
                  <li
                    key={j}
                    className="flex gap-4 items-start text-[#D4D4D8] text-sm leading-relaxed font-light"
                  >
                    <MdCheckCircle
                      size={18}
