  ...
  return (
    ...
    >
      <div className="bg-bg-dark rounded-[31px] p-12 md:p-14 text-center h-full relative overflow-hidden backdrop-blur-xl flex flex-col justify-between">
        <div className={`absolute top-0 left-0 w-full h-full opacity-10 blur-3xl ${reward.color}`} />
        <div className="relative z-10 flex flex-col h-full">
          <div className="flex justify-center mb-8">
            <div className={`w-20 h-20 rounded-full flex items-center justify-center bg-bg-dark border border-white/10 shadow-[0_0_30px_rgba(255,255,255,0.05)]`}>
              {(() => {
                const RankIcon = reward.rankIcon;
                return <RankIcon size={36} className="text-white opacity-90" strokeWidth={1} />;
              })()}
            </div>
          </div>
          
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-white/50 mb-6">{reward.title}</div>
          ...
          <div className="flex-1 flex flex-col justify-center">
            ...
            <div className="text-lg font-light text-white/70 mb-8">{reward.subtitle}</div>
          </div>
...
const Rewards = () => {
  ...
  return (
    <SectionWrapper id="rewards" className="section-dark-2 relative overflow-hidden">
      ...
      <div ref={sectionRef} className="section-inner">
        ...
        >
          <span className="glass-pill mb-6 text-primary border-primary/30">PRIZES & FUNDING</span>
          <h2 className="heading-display text-5xl md:text-6xl lg:text-7xl xl:text-8xl mt-4">
            Tournament <span className="italic font-serif text-white/60">Rewards</span>
          </h2>
          <p className="text-text-secondary mt-6 max-w-2xl mx-auto text-xl font-light">
            Top performers receive immense capital to scale their verified edges.
          </p>
        </motion.div>

        <div className="grid md:grid-cols-3 gap-8 mb-24">
          {TOP_REWARDS.map((reward, i) => (
            <RewardCard key={i} reward={reward} index={i} isVisible={isVisible} />
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={isVisible ? { opacity: 1 } : {}} transition={{ delay: 0.6 }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-[1000px] mx-auto">
            {EXTRA_REWARDS.map((reward, i) => (
              ...
              >
                <div className="text-2xl md:text-3xl font-serif font-light text-white group-hover:text-primary transition-colors">{reward.amount}</div>
                <div className="text-xs text-white/50 uppercase tracking-widest mt-3">{reward.label}</div>
              </motion.div>
            ))}
          </div>
        </motion.div>
