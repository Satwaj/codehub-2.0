...
import { Instagram, Twitter, Youtube, GraduationCap, Star } from 'lucide-react';
...
const Mentors = () => {
  ...
  return (
    <SectionWrapper id="mentors" className="section-dark relative text-center">
      ...
      <div ref={sectionRef} className="section-inner z-10 max-w-[1000px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} className="mb-24">
          <span className="glass-pill mb-6 text-primary border-primary/30">YOUR MENTORS</span>
          <h2 className="heading-display text-5xl md:text-6xl text-white mt-4">
            Learn from the <span className="italic font-serif text-white/50">Absolute Best.</span>
          </h2>
        </motion.div>
          ...
          {MENTORS.map((mentor, i) => (
            ...
            >
              <div className="relative w-full aspect-square md:aspect-[4/3] rounded-2xl overflow-hidden mb-8">
                ...
                <img src={mentor.image} alt={mentor.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 group-hover:scale-105" />
                <div className="absolute bottom-6 left-6 z-20 text-left">
                  <h3 className="text-3xl font-serif text-white mb-2">{mentor.name}</h3>
                  <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/20 backdrop-blur-md rounded-full border border-primary/30 text-primary text-xs font-bold uppercase tracking-widest">
                    <GraduationCap size={14} /> {mentor.role}
                  </div>
                </div>
              </div>
              ...
              <div className="text-left px-4">
                <div className="flex items-center gap-2 text-text-secondary text-sm font-semibold tracking-wider uppercase mb-4">
                  <Star size={16} className="text-primary text-opacity-80" /> {mentor.stats}
                </div>
                <p className="text-white/70 font-light leading-relaxed mb-8">{mentor.desc}</p>
                
                <div className="flex gap-4">
                  {mentor.social.ig && <a href={mentor.social.ig} className="text-white/40 hover:text-white transition-colors"><Instagram size={20} /></a>}
                  {mentor.social.tw && <a href={mentor.social.tw} className="text-white/40 hover:text-white transition-colors"><Twitter size={20} /></a>}
                  {mentor.social.yt && <a href={mentor.social.yt} className="text-white/40 hover:text-white transition-colors"><Youtube size={20} /></a>}
                </div>
              </div>
            </motion.div>
          ))}
