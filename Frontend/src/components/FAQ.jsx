...
const FAQItem = ({ faq, isOpen, onClick }) => {
  return (
    <div className="border border-white/10 rounded-2xl bg-white/5 overflow-hidden mb-4 transition-colors hover:border-white/20">
      ...
      >
        <span className="text-lg md:text-xl font-medium text-white/90 pr-8">{faq.q}</span>
        <div className={`w-8 h-8 rounded-full border border-white/10 flex items-center justify-center flex-shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 bg-white/10' : ''}`}>
          <ChevronDown size={18} className={isOpen ? 'text-primary' : 'text-white/50'} />
        </div>
      </button>
      <AnimatePresence>
        {isOpen && (
    ...
    <SectionWrapper id="faq" className="section-dark-2 relative">
      ...
      <div ref={sectionRef} className="section-inner z-10 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} className="text-center mb-20">
          <span className="glass-pill mb-6 text-primary border-primary/30">FAQ</span>
          <h2 className="heading-display text-5xl md:text-6xl text-white mt-4">
            Commonly Asked <span className="italic font-serif text-white/50">Questions</span>
          </h2>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={isVisible ? { opacity: 1 } : {}} transition={{ delay: 0.4 }} className="mt-16 text-center">
          <p className="text-text-secondary text-sm md:text-base font-light italic inline-flex items-center gap-3">
            <MessageCircle size={18} className="text-primary" /> Still have questions? Hit us up in the admissions discord.
          </p>
        </motion.div>
      </div>
    </SectionWrapper>
