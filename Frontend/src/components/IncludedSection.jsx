    ...
    <SectionWrapper className="section-dark relative">
      ...
      <div ref={sectionRef} className="section-inner z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} className="text-center mb-24">
          <span className="glass-pill mb-6 text-primary border-primary/30">LOGISTICS</span>
          <h2 className="heading-display text-5xl md:text-6xl lg:text-7xl mt-4 text-white">
            What's <span className="italic font-serif text-white/50">Included?</span>
          </h2>
        </motion.div>
        ...
        <div className="flex flex-col md:flex-row justify-center gap-10 max-w-4xl mx-auto">
          ...
          >
            ...
            <ul className="space-y-6 relative z-10">
              {INCLUDED.map((item, i) => (
                <li key={i} className="flex gap-4 items-center text-white/80 font-light text-lg">
                  <Check size={18} className="text-green-500 flex-shrink-0" strokeWidth={2} />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
          ...
          {/* Not Included */}
          <motion.div initial={{ opacity: 0, x: 30 }} animate={isVisible ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }}
          ...
          >
            <div className="flex items-center gap-4 mb-10">
              ...
              <h3 className="text-3xl font-serif text-white/60">Not Included</h3>
            </div>
            <ul className="space-y-6">
              {NOT_INCLUDED.map((item, i) => (
                <li key={i} className="flex gap-4 items-center text-white/50 font-light text-lg">
                  <X size={18} className="text-red-500/50 flex-shrink-0" strokeWidth={2} />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
